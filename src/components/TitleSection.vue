...
<template>
    <div
        class="min-h-full mx-6 flex flex-col justify-around md:items-center md:text-center md:max-w-screen-md md:mx-auto">
        <div><!---spacer--></div>
        <div class="flex flex-col gap-2">
            ...
            <div class="flex gap-2 md:justify-center">
                <a v-for="social in socials" :href="social.link" :title="social.name"
                    class="bg-arisu-700 rounded-full p-3 text-arisu-100">
                    <component :is="social.icon" class="w-7 h-7 flex items-center" />
                </a>
            </div>
        </div>
        <button>
            <ArrowDownCircleIcon class="h-10 w-full" />
        </button>
    </div>
</template>